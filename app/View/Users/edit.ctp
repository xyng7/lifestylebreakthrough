...
<div class="actions">
    ...
    <ul>
        ...
        if(($id=$this->Session->read('Auth.User.id')) !==  ($this->Form->value('User.id'))){
            echo $this->Form->postLink(__('Archive'), array('action' => 'delete', $this->Form->value('User.id')), null, __('Are you sure you want to archive %s?', $this->Form->value('User.username'))); ?></li>
         <?php  } ?>
